...
listaalpha=[0.2,0.3,0.4,0.5]
listaitergrasp=[4,8,16]
replicas=4
listatext=os.listdir('Instancies')
listasol=list()
f.write("instancia, alpha, numeroiteracionesgrasp, replica, valoroptimo, valorobjetivo, iteracionmejorsolucion, tiempohallarmejorsolucion, tiempototal \n")
for i in listatext:
	for alpha in listaalpha:
		for a in range(0,replicas):
			for itergrasp in listaitergrasp:
				...
				f.write(str(i)+str(" ")+str(alpha)+str(" ")+str(itergrasp)+str(" ")+str(a+1)+str(" ")+str(s[6])+str(" ")+str(s[1])+str(" ")+str(s[2]+1)+str(" ")+str(s[4])+str(" ")+str(tiempotot)) #(nombre de la instancia, valor alpha, num iter grasp, numero de replica, valor objetivo, iteracion que hallo la mejor, tiempo en hallar la mejor sol, tiempo en ejecutar)
				f.write("\n")
